...
新问题：把op当成类方法，对于一些算子，需要指明axis，算子类只能拿到input和上游梯度，能否求出axis？

利用上游梯度可以得到输出的形状，这类算子一般是单tensor输入，对比shape可以得到可能的axis

但是对于 (2,2)->(2,) 难以计算，所以需要新信息，最好直接指明axis。

修改装饰器，把axis和keepdim作为**kwarg 进行传递，并在输出的tensor中存储这两个量。
